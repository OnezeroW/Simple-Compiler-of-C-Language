...
void parse::lalr_analyze(spie s)
{
    ...
    else if(nextaction == 'r')
    {
        ...
        out <<endl;
        semantic_analyze(tempstate);  //语义分析
...
void parse ::  readlalr()
{
    ifstream fin;
    string str;
    fin.open("lr.txt", ios_base::in);
    if (!fin.is_open())
    {
        cout << "Open lr.txt Faild" << endl;
        exit(1);
    }
...
void parse :: semantic_analyze(int label)
{
    ...
    case 7:
        ...
        codestack.pop();
        tempnode4.code.push_back(randommake()+"jmp L" + tempnode1.result);  //跳回
        itoa(nownum,buf,10);
